...
const filteredItems = computed(() => {
    return item.value.filter(it => {
        const matchesClassification = !selectedClassification.value || it.mealKitClassificaion === selectedClassification.value;
        ...
        return matchesClassification && matchesFoodClassification && it.mealKitCount < 20;
    });
});
...
const sortedItems = computed(() => {
    return [...filteredItems.value].sort((a, b) => {
        if (sortOrder.value === 'asc') {
            return a.mealKitCount - b.mealKitCount;
        } else {
            return b.mealKitCount - a.mealKitCount;
        }
    });
});
...
// 분류 및 음식 분류 옵션 추출
const classifications = computed(() => [...new Set(item.value.map(it => it.mealKitClassificaion))]);
const foodClassifications = computed(() => [...new Set(item.value.map(it => it.mealKitFoodClassificaion))]);
